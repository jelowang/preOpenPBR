...
void MMTCheckOverflow () {
	...
	for ( looper = mem_monitor.next ; looper ; looper=looper->next ) 
	{
		...
		for ( counter = 0 ; counter < MMT_MASK_LENGTH ; counter ++ )
		{
			...
			{
				overtimes ++ ;
				printf ( "!!!!! M : %x , In : '%s' , At line : '%d' - overflowed" , (void* )looper->address , looper->file , looper->line ) ;
				free ( mask ) ;
				return ;
			}
		}		
...
/*
 brief : 
*/
void* H3MemoryRealloc ( void* buffer , long long length ) 
{
	void* address = 0 ;
	...
	# ifdef H3MEMORY_ENABLE_MEMORY_CHECK
		...
		MMTAdd ( "SCRealloc" , -1 , length , (long long)address ) ;
		H3MemoryCheckOverflowed () ;
	# else
...
/*
 brief : 
*/
void* H3MemoryRemalloc ( void* buffer , long long bufferlen , long long length )  
{
	...
	# ifdef H3MEMORY_ENABLE_MEMORY_CHECK
		...
		MMTAdd ( "SCRemalloc" , -1 , length , (long long)memory ) ;
		H3MemoryCheckOverflowed () ;
	# else
...
void H3MemoryCheckLeaked () 
{
	# ifdef H3MEMORY_ENABLE_MEMORY_CHECK
		...
		for ( ;walker;walker=walker->next) {
			...
			printf ("# M : %x , S : %d , In '%s' , At line : '%d' - leaked",
				(void* )walker->address,\
				walker->length,\
				walker->file,\
				walker->line\
		...
		printf ( "Leaked Totall : %1.3f kb" , (float )totall/1024.0 ) ;
		printf ( "Leaked Times : %d" , leakedtimes ) ;
...
void H3MemoryCheckTotallMemoryUsed () 
{
	# ifdef H3MEMORY_ENABLE_MEMORY_CHECK
		...
		printf ( "Totall Memory Used : %1.3f kb" , (float )totall/1024.0 ) ;
