...
Cone::Cone ( const Transform* o2w , const Transform* w2o , bool ro , float ht , float rad , float tm ) : Shape(o2w, w2o, ro) {
    radius = rad ;
    height = ht ;
...
Cone* Cone::Create ( const Transform* o2w , const Transform* w2o , bool reverseOrientation , Parameter* params ) {

    float radius = params->getFloat ( "radius" , 1.0f ) ;
	float height = params->getFloat ( "height" , 1.0f ) ;
    float phimax = params->getFloat ( "phimax" , 360.0f ) ;
    Cone* cone = new Cone ( o2w , w2o , reverseOrientation , height , radius , phimax ) ;
	return cone ;

}

BBox Cone::ObjectBound () const {
    Point p1 = Point ( -radius , -radius , 0.0f ) ;
    Point p2 = Point (  radius , radius , height ) ;
    BBox box( p1, p2 ) ;
	return box ;
}

bool Cone::Intersect ( const Ray& r , float* tHit , float* rayEpsilon , DifferentialGeometry* dg ) const {
...
float Cone::Area () const {
    return radius * sqrtf((height*height)+(radius*radius)) * phiMax / 2.f ;
}
